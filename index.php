...
function fetchReviewsPreview(mysqli $conn, string $productCode, int $limit = 3): array {
  ...
  try {
    ...
    if ($stmt === false) {
      error_log('MySQL prepare failed in fetchReviewsPreview: ' . $conn->error . ' SQL: ' . $sql);
      return $out;
    }
...
// Cart count
// Determine cart count: prefer DB-backed cart for logged-in users, fall back to session cart
$cartCount = 0;
if (isset($_SESSION['user_id'])) {
  $session_user = $_SESSION['user_id'];
  $resolved_user_id = null;
  if (is_int($session_user) || (is_string($session_user) && ctype_digit($session_user))) {
    $resolved_user_id = (int)$session_user;
  } else {
    // try lookup by email
    ...
    if ($maybe_email !== '') {
      try {
        ...
        if ($s !== false) {
          $s->bind_param('s', $maybe_email);
          $s->execute();
          $r = $s->get_result();
          if ($row = $r->fetch_assoc()) $resolved_user_id = (int)$row['id'];
          $s->close();
        }
      } catch (Throwable $t) { /* ignore */ }
    }
  ...
  if ($resolved_user_id !== null) {
    try {
      $q = $conn->prepare("SELECT SUM(quantity) AS cnt FROM cart WHERE user_id = ?");
      if ($q !== false) {
        $q->bind_param('i', $resolved_user_id);
        $q->execute();
        $res = $q->get_result();
        $row = $res ? $res->fetch_assoc() : null;
        $cartCount = isset($row['cnt']) ? (int)$row['cnt'] : 0;
        $q->close();
      }
    } catch (Throwable $t) { /* ignore */ }
  }
}
if ($cartCount === 0 && !empty($_SESSION['cart']) && is_array($_SESSION['cart'])) {
...
<body>
  ...
  <main class="site-main container">
    ...
    <?php foreach ($cats as $category => $page): ?>
      ...
      if ($stmt === false) {
        error_log('MySQL prepare failed in index product fetch: ' . $conn->error);
