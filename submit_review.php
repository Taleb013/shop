...
try {
    ...
    // checked prepare helper
    $checked_prepare = function(mysqli $m, string $sql) {
        $stmt = $m->prepare($sql);
        if ($stmt === false) {
            error_log('MySQL prepare failed in submit_review: ' . $m->error . ' SQL: ' . $sql);
    ...
    $stmt = $checked_prepare($conn, "INSERT INTO reviews (product_code, user_name, rating, comment, created_at) VALUES (?, ?, ?, ?, NOW())");
